...
def test_opts_out_dir_given(tmp_path):
    ...
    #  Short option.
    opts = index_images.get_opts([str(p1), "-d", str(p2)])
    assert str(opts.html_path) == str(p2 / index_images.DEFAULT_OUTPUT_NAME)

    #  Long option.
    opts = index_images.get_opts([str(p1), "--out-dir", str(p2)])
    assert str(opts.html_path) == str(p2 / index_images.DEFAULT_OUTPUT_NAME)
...
def test_opts_out_name_given(tmp_path):
    ...
    #  Short option.
    opts = index_images.get_opts([str(p1), "-n", "other_name.html"])
    assert str(opts.html_path) == str(p1 / "other_name.html")

    #  Long option.
    opts = index_images.get_opts([str(p1), "--name", "other_name.html"])
    assert str(opts.html_path) == str(p1 / "other_name.html")
